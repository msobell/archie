...
def reciprocal_rank_fusion(fts_ids: List[int], vec_ids: List[int], k: int = 60) -> List[Tuple[int, float]]:
...
def hybrid_search(session: Session, query: str, n_results: int = 10) -> List[Memory]:
    ...
    try:
        ...
        logger.warning(f"Vector search unavailable, falling back to FTS only: {e}")
